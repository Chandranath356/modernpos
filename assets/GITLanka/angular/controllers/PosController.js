...
function (
    ...
    $scope.purchasePrice    = 0;   
    $scope.totalAmount      = 0;
    $scope.discountAmount   = 0;
    $scope.singlediscountAmount   = 0;
    $scope.purchasePrice    = 0;
    $scope.taxAmount        = 0;
    ...
    $scope.singlediscountType = 'plain';
    ...
    $scope._calcTotalPayable = function ($childScope) {
        ...
        if ($scope.payable != 0 && (discountPercentage >= $scope.payable)) {
            $scope.discountAmount = 0;
            $scope.discountInput = 0;
            if (window.store.sound_effect == 1) {
                window.storeApp.playSound("error.mp3");
        ...
        if($scope.payable != 0 && (($scope.payable - discountPercentage) <= ($scope.purchasePrice*(110/100)))){
            ...
            window.toastr.error("payable amount must be greater than product purchaseprice", "Warning!");   
        }
    ...
    $scope.SingleProductDiscount = function(productindex){
        
        $scope._calcsingleproductdiscount(productindex);
        
    }

    $scope._calcsingleproductdiscount = function(productindex){
     
       var singleItem = $scope.itemArray[productindex];
       var total = $scope.singlediscountInput;
    
         
       if (window._.includes($scope.singlediscountInput[productindex], '%')) {

           $scope.singlediscountType = "percentage";
        ...
        if($scope.singlediscountInput[productindex] < 1){
            
            $scope.singlediscountInput[productindex] = 0;
             
            $scope.singlediscountAmount   = 0;

        }else{
           
           $scope.singlediscountAmount   = parseFloat($scope.singlediscountInput[productindex]);;
           
        }

      if ($scope.singlediscountType == 'percentage') {

       var singlediscountPercentage =  parseFloat($scope._percentage(singleItem.subTotal, $scope.singlediscountAmount));
            
      } else {

         singlediscountPercentage =  parseFloat($scope.singlediscountAmount); 
      }
      
      $scope.totaldiscount[productindex] = singlediscountPercentage;
      
      if(singlediscountPercentage < 1){
         $scope.singlediscountInput[productindex] = 0;
         singlediscountPercentage = 0;
      }

      $scope.singleAmountPaybale = singleItem.subTotal + singleItem.taxamount; 
      
      document.getElementById("subTotal-"+productindex).innerHTML = (singleItem.subTotal + singleItem.taxamount - singlediscountPercentage).toFixed(2);
      var amountpayafterdis = ($scope.singleAmountPaybale - singlediscountPercentage);
      

      if((singleItem.totalpurchasePrice*(110/100)) > amountpayafterdis){
         $scope.singlediscountInput[productindex] = 0;
         singlediscountPercentage = 0;
        if (window.store.sound_effect == 1) {
       ...
       var discount = 0;
       for(var i=0;i<$scope.totaldiscount.length;i++){ 
          discount += parseInt($scope.totaldiscount[i]);
       }
       
       $scope.discountInput = discount;
       $scope._calcTotalPayable();
    }
    ...
    $scope.addItemToInvoice = function (id, qty, index) {
        ...
        then(function(response) {
            ...
            if (response.data.p_id) {
                ...
                if (find) {
                    ...
                    window._.map($scope.itemArray, function (item) {
                        if (item.id == response.data.p_id) {
                            ...
                            $scope.purchasePrice = $scope.purchasePrice+parseFloat(response.data.purchase_price);
                            item.totalpurchasePrice = item.totalpurchasePrice+parseFloat(response.data.purchase_price);
                    ...
                    item.pcode = response.data.p_code;
                    ...
                    item.totalpurchasePrice =parseFloat(response.data.purchase_price);
                    item.price = parseFloat(response.data.sell_price) + additonalTaxAmount;
                    item.purchasePrice = parseFloat(response.data.purchase_price);
                    item.quantity = qty;
                    ...
                    $scope.singlediscountInput.push(0);
                    JSON.parse(JSON.stringify($scope.singlediscountInput));
                ...
                if(index !== undefined){
                   $scope._calcsingleproductdiscount(index);
                }
    ...
    $scope.DecreaseItemFromInvoice = function (id,index) {
                ...
                window._.map($scope.itemArray, function (item) {
                    ...
                    if (item.id == id) {
                        if (item.quantity > 1) {
                            ...
                            $scope.totalQuantity = $scope.totalQuantity - qty;
                            item.totalpurchasePrice = item.totalpurchasePrice - parseFloat(item.purchasePrice);
                            $scope.purchasePrice = $scope.purchasePrice - parseFloat(item.purchasePrice);
    ...
    $scope.removeItemFromInvoice = function (index, id) {
        $scope.singlediscountInput.splice(index, 1);
        $scope.totaldiscount.splice(index, 1);
       ...
       $scope.discountInput = discount;
        ...
        window._.map($scope.itemArray, function (item, key) {
            ...
            if (item.id == id) {
                ...
                $scope.purchasePrice = $scope.purchasePrice - parseFloat(item.totalpurchasePrice);
    ...
    $scope.payNow = function() {
        ...
        for(var i=0;i<$scope.singlediscountInput.length;i++){
              $scope.singlediscountInput[i] = 0;
        }
        if ($scope.itemArray.length <= 0) {
            if (window.store.sound_effect == 1) {
                window.storeApp.playSound("error.mp3");
