...
function (
        ...
        "buttons": [
            {
                ...
                customize: function ( win ) {
                    $(win.document.body)
                        .css( 'font-size', '10pt' )
                        .append(
                            '<div><b><i>Developed & Maintained by: GITLanka.com</i></b></div>'
                        )
                        .prepend(
                            '<div class="dt-print-heading"><img class="logo" src="'+baseUrl+'/assets/GITLanka/img/logo-favicons/1_logo.png"/><h2 class="title">'+window.store.name+'</h2><p><b><i>From</i> '+window.from +' <i>To</i> '+window.to+'</b><br>Printed on: '+window.formatDate(new Date())+'</p></div>'
    ...
    $(document).delegate("#reset", "click", function (e) {
        ...
        $("#to_store_id").val("").trigger("change");
        $("#image_thumb img").attr("src", "../assets/GITLanka/img/noimage.jpg");
        $("#image").val("");
