	...
	public function editProduct($product_id, $data) 
	{
    	...
    	if (isset($data['product_store'])) {
			...
			//--- product to store ---//
              ...
              if($data['old_price'] == $data['sell_price']){
                  ...
                  $updated_date = null;
                    ...
                    $upadate_date = $data['updated_date'];
              }
              ...
              // echo $upadate_date;
			    ...
			    if (!$product) {
			    	...
			    	$statement = $this->db->prepare("UPDATE `product_to_store` SET `sup_id` = ?, `brand_id` = ?, `box_id` = ?, `taxrate_id` = ?, `tax_method` = ?, `preference` = ?, `purchase_price` = ?, `sell_price` = ?, `e_date` = ?, `alert_quantity` = ?, `old_product_price` = ?,`updated_date`=?,`alert_quantity` = ? WHERE `store_id` = ? AND `product_id` = ?");
					$statement->execute(array($data['sup_id'], $data['brand_id'], $data['box_id'], $data['taxrate_id'], $data['tax_method'], $preference, $data['purchase_price'], $data['sell_price'], $data['e_date'], $data['alert_quantity'],$old_price,$upadate_date, $data['alert_quantity'], $store_id, $product_id));
			    }
