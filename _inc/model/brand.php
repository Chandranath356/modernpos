...
class ModelBrand extends Model 
{
	...
	{
    	$statement = $this->db->prepare("INSERT INTO `brands` (category_id,brand_name, code_name, brand_details, brand_image, created_at) VALUES (?,?, ?, ?, ?, ?)");
    	$statement->execute(array($data['category_id'],$data['brand_name'], $data['code_name'], $data['brand_details'], $data['brand_image'], date_time()));
	...
	{
    	$statement = $this->db->prepare("UPDATE `brands` SET `brand_name` = ?,`category_id` = ?, `code_name` = ?, `brand_details` = ?, `brand_image` = ? WHERE `brand_id` = ? ");
    	$statement->execute(array($data['brand_name'],$data['category_id'], $data['code_name'], $data['brand_details'], $data['brand_image'], $brand_id));
