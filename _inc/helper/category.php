...
function get_category_name($id){
  ...
  $model = registry()->get('loader')->model('category');
  return $model->getCategoryName($id);
}
...
function get_subcategorys($data = array(), $store = null) 
{	
	$model = registry()->get('loader')->model('subcategory');
	return $model->getSubCategorys($data, $store);
}
